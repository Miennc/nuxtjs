      ...
      <div class="grid lg:grid-cols-7 grid-cols-1 gap-4 space-x-5">
        ...
        <div class="col-span-4 space-y-7">
          ...
          <div class="flex space-x-2 items-center font-bold justify-center ">
            ...
            <button class="border border-gray-800 text-black p-2 text-xl">Add To
              Cart
            </button>
          </div>
        </div>
...
export default {
  ...
  async fetch() {
    try {
      const response = await this.$repositories.productDetail.getDataProductDetails(this.$route.query.id);
      this.cartProducts = response.product;
      this.mightYouLikeProducts = response.mightYouLikeProducts;
